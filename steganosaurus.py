...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Encode/Decode text into/from an image, using a save key. The save key must be at least 5 '
                    'characters long and can be made up of uppercase and lowercase letters, numbers, and special '
                    'characters.', argument_default='--help')
    parser.add_argument('image input', metavar='filepath', type=str, nargs="?", help='Filepath of image to be encoded.')
    parser.add_argument('text input', metavar='text', type=str, nargs="?", help='Text to be encoded.')
    parser.add_argument('save key', metavar='key', type=str, nargs="?",
                        help='Key to encrypt/decrypt message into image.')
    parser.add_argument('-e', '--encode', dest='', action='store', help='Encode the given text into an image.', nargs=3)
    parser.add_argument('-d', '--decode', dest='', action='store', help='Decode the text within in image.', nargs=2)
    ...
    try:
        ...
        assert action in ['--encode', '-e', '--decode', '-d'], \
            "Action is not --encode (-e) or --decode (-d): " + action

        process(userinputs, action)
    except:
        parser.print_usage()
        # raise IOError("Proper input format is --encode or --decode followed by a string and then a save key. "
        #               "The save key must be at least five characters long and can be made up of uppercase and "
        #               "lowercase letters, numbers, and special characters.")


def process(filename, action):
    if action == '--encode' or action == '-e':
        return encode_text(filename[0], " ".join(filename[1:-1]), filename[-1])
